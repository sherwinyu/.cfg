...
alias cdxyk='codex --dangerously-bypass-approvals-and-sandbox --model spark -c service_tier=fast'
...
alias sbrd='rr && cd packages/agent-server && bun run dev'   # server bun run dev
alias wbrd='rr && cd packages/web-ui-react && bun run dev'   # web bun run dev
